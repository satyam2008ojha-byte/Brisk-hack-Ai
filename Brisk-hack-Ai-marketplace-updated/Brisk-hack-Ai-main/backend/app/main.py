...
from .models import (
    User, Event, Registration, Team, TeamMember, Submission, Notification,
    MarketplaceListing, MarketplaceBooking,
)
from .schemas import (
    Signup, Login, EventCreate, TeamCreate, SubmissionCreate, ScoreUpdate,
    MarketplaceListingCreate, MarketplaceBookingCreate, MarketplaceBookingStatusUpdate,
)
...
MARKETPLACE_CATEGORIES = {"sponsor", "venue", "food", "merch"}
BOOKING_STATUSES = {"pending", "accepted", "rejected", "completed", "cancelled"}


def listing_payload(item: MarketplaceListing):
    return {
        "id": item.id,
...
def booking_payload(item: MarketplaceBooking):
    return {
        "id": item.id,
        "listing_id": item.listing_id,
        "listing_name": item.listing.name if item.listing else "",
...
def marketplace_listings(category: str = "", search: str = "", db: Session = Depends(get_db)):
    query = db.query(MarketplaceListing).filter(MarketplaceListing.is_active.is_(True))
    if category:
        category = category.lower().strip()
        if category not in MARKETPLACE_CATEGORIES:
            raise HTTPException(400, "Invalid marketplace category")
        query = query.filter(MarketplaceListing.category == category)
    if search:
        term = f"%{search.strip()}%"
        query = query.filter(
            (MarketplaceListing.name.ilike(term))
            | (MarketplaceListing.description.ilike(term))
            | (MarketplaceListing.location.ilike(term))
        )
    rows = query.order_by(MarketplaceListing.created_at.desc()).all()
    return [listing_payload(item) for item in rows]
...
def create_marketplace_listing(
    ...
    user: User = Depends(require_role("organizer", "sponsor")),
):
    category = data.category.lower().strip()
    if category not in MARKETPLACE_CATEGORIES:
        raise HTTPException(400, "Category must be sponsor, venue, food, or merch")
    values = data.model_dump()
    values["category"] = category
    values["contact_email"] = str(data.contact_email or user.email)
    item = MarketplaceListing(owner_id=user.id, **values)
    db.add(item)
    db.commit()
    db.refresh(item)
    return {"message": "Marketplace listing created", "listing": listing_payload(item)}


@app.get("/api/marketplace/{listing_id}")
def marketplace_listing_details(listing_id: int, db: Session = Depends(get_db)):
    item = db.query(MarketplaceListing).filter(MarketplaceListing.id == listing_id, MarketplaceListing.is_active.is_(True)).first()
    if not item:
        raise HTTPException(404, "Marketplace listing not found")
    return listing_payload(item)


@app.post("/api/marketplace/{listing_id}/book")
def book_marketplace_listing(
    listing_id: int,
    data: MarketplaceBookingCreate,
    db: Session = Depends(get_db),
    user: User = Depends(current_user),
):
    listing = db.query(MarketplaceListing).filter(MarketplaceListing.id == listing_id, MarketplaceListing.is_active.is_(True)).first()
    if not listing:
        raise HTTPException(404, "Marketplace listing not found")
    ...
    booking = MarketplaceBooking(
        listing_id=listing.id, customer_id=user.id, **data.model_dump()
    )
    db.add(booking)
    db.flush()
    db.add(Notification(
        ...
        message=f"{user.name} requested {listing.name}. Booking #{booking.id}",
    ))
    db.add(Notification(
        user_id=user.id,
        title="Booking request sent",
        message=f"Your request for {listing.name} is pending.",
    ))
    db.commit()
    db.refresh(booking)
    return {"message": "Booking request sent", "booking": booking_payload(booking)}


@app.get("/api/marketplace/bookings/me")
def my_marketplace_bookings(db: Session = Depends(get_db), user: User = Depends(current_user)):
    rows = db.query(MarketplaceBooking).filter(MarketplaceBooking.customer_id == user.id).order_by(MarketplaceBooking.created_at.desc()).all()
    return [booking_payload(item) for item in rows]


@app.get("/api/marketplace/bookings/received")
def received_marketplace_bookings(db: Session = Depends(get_db), user: User = Depends(current_user)):
    rows = (
        db.query(MarketplaceBooking)
        .join(MarketplaceListing, MarketplaceListing.id == MarketplaceBooking.listing_id)
        .filter(MarketplaceListing.owner_id == user.id)
...
@app.patch("/api/marketplace/bookings/{booking_id}")
def update_marketplace_booking(
    booking_id: int,
    data: MarketplaceBookingStatusUpdate,
    db: Session = Depends(get_db),
    user: User = Depends(current_user),
):
    status_value = data.status.lower().strip()
    ...
    is_provider = booking.listing.owner_id == user.id
    is_customer = booking.customer_id == user.id
    if not is_provider and not is_customer:
        raise HTTPException(403, "You cannot update this booking")
    if is_customer and status_value != "cancelled":
        raise HTTPException(403, "Customers can only cancel bookings")
    if is_provider and status_value == "cancelled":
        raise HTTPException(400, "Provider should reject instead of cancel")
    booking.status = status_value
    db.add(Notification(
        user_id=booking.customer_id,
        title="Marketplace booking updated",
        message=f"Booking #{booking.id} is now {status_value}.",
    ))
    db.commit()
    db.refresh(booking)
    return {"message": "Booking status updated", "booking": booking_payload(booking)}
